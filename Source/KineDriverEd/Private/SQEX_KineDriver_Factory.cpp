...
uint32 USQEX_KineDriver_Factory::GetMenuCategories() const { return EAssetTypeCategories::Physics; }
UObject * USQEX_KineDriver_Factory::FactoryCreateNew(UClass * InClass, UObject * InParent, FName InName, EObjectFlags flags, UObject * Cntext, FFeedbackContext * Warn)
{
	return NewObject<USQEX_KineDriverData>(InParent, InClass, InName, flags);
}
UObject* USQEX_KineDriver_Factory::FactoryCreateBinary(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const uint8*& Buffer, const uint8* BufferEnd, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
...
bool USQEX_KineDriver_Factory::CanReimport(UObject* Obj, TArray<FString>& OutFilenames)
{
	USQEX_KineDriverData* ImportedAsset = Cast<USQEX_KineDriverData>(Obj);
	if (ImportedAsset)
	{
		//OutFilenames.Add(FPaths::ConvertRelativePathToFull(FPaths::ProjectDir(), ImportedAsset->SourceFile));
		return true;
	}
	return false;
}
void USQEX_KineDriver_Factory::SetReimportPaths(UObject* Obj, const TArray<FString>& NewReimportPaths)
{
	USQEX_KineDriverData* ImportedAsset = Cast<USQEX_KineDriverData>(Obj);
...
EReimportResult::Type USQEX_KineDriver_Factory::Reimport(UObject* Obj)
{
	USQEX_KineDriverData* ImportedAsset = Cast<USQEX_KineDriverData>(Obj);
	if (!ImportedAsset) { return EReimportResult::Failed; }
	//const FString Filename = FPaths::ConvertRelativePathToFull(FPaths::ProjectDir(), ImportedAsset->SourceFile);
